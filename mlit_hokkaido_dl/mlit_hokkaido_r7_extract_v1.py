...
OUTPUT_JSON = "hokkaido_r7_all_full.json"
...
def parse_excel(path, kind, col_map):
    ...
    for i in range(data_start, len(df)):
        ...
        # 契約金額決定
        final_amount = None
        if contract_amount_col and is_winner_by_zui:
...
def clean_company(c, project=""):
    ...
    # 北海道R7: JV名に案件名が混入するケースを補正
    c = c.replace("雨竜川ダム再生事業の内雨竜第２ダム堤体建設第１期工事", "")
    c = c.replace("北海道縦貫自動車道七飯町大沼トンネル西大沼工区工事", "")
    c = c.replace("北海道縦貫自動車道　七飯町　大沼トンネル西大沼工区工事", "")
    c = c.replace("北海道縦貫自動車道七飯町大沼トンネル峠下工区工事", "")
    c = c.replace("北海道縦貫自動車道　七飯町　大沼トンネル峠下工区工事", "")
    c = c.replace("一般国道２３９号苫前町苫前トンネル工事", "")
    c = c.replace("一般国道２３９号　苫前町　苫前トンネル工事", "")
    c = c.replace("特定ＪＶ", "特定JV")
    c = c.replace("特定Ｊ", "特定JV")
    c = c.replace("共同企業体業体", "共同企業体")
    c = c.replace("設計共同体体", "設計共同体")
...
def main():
    print("=" * 70)
    print("北海道開発局 R7統合Excel → JSON抽出 v1")
    print("=" * 70)
    
    # 北海道R7: 月別Excel 24本を一括処理
    kouji_col_map = {
        'memo': 18,
        'contract_amount': 17,
