...
OUTPUT_JSON = "tohoku_r7_all_full.json"
...
def parse_excel(path, kind, col_map):
    ...
    for i in range(data_start, len(df)):
        ...
        predicted = to_int(row[predicted_col]) if len(row) > predicted_col else None
        rate_pct = round(final_amount / predicted * 100, 2) if predicted else 0
        
        company = str(row[company_col]).strip() if len(row) > company_col and pd.notna(row[company_col]) else ''
        ...
        records.append({
            ...
            'project': project,
            'company': company,
            'method': str(row[method_col]).strip() if len(row) > method_col and pd.notna(row[method_col]) else '',
            'type_kind': str(row[type_col]).strip() if len(row) > type_col and pd.notna(row[type_col]) else '',
            'is_sougou': str(row[sougou_col]).strip() if len(row) > sougou_col and pd.notna(row[sougou_col]) else '',
            'bid_date': str(row[bid_col])[:10] if len(row) > bid_col and pd.notna(row[bid_col]) else '',
            'contract_date': str(row[contract_col])[:10] if len(row) > contract_col and pd.notna(row[contract_col]) else '',
...
def clean_company(c, project=""):
    ...
    # 多摩川トンネル羽田立坑JVだけ、会社欄に旧案件名が混入
    c = c.replace("Ｒ２国道３５７号多摩川トンネル羽田立坑工事", "")

    c = c.replace("設計共同体体", "設計共同体")

    # 東北R7: 会社欄に工事件名が混入するJVを補正
    c = c.replace("成瀬ダム堤体打設工事", "")
...
def main():
    print("=" * 70)
    print("東北地方整備局 R7統合Excel → JSON抽出 v1")
    print("=" * 70)
    
    # 東北R7: 月別Excel 24本を一括処理
    kouji_col_map = {
        'memo': 22,
        'contract_amount': 20,
        'amount_1': 14, 'amount_2': 16, 'amount_3': 18,
    }
    ...
    files = sorted([p for p in Path("r7_full").glob("*.xlsx") if not p.name.startswith("._")])
    kouji = []
    gyoumu = []

    for f in files:
        preview = pd.read_excel(f, header=None, engine="openpyxl", nrows=8)
