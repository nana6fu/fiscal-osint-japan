...
RAW_DIR = Path("data/mod/chugoku_shikoku/raw")
OUT_DIR = Path("data/mod/chugoku_shikoku/processed")
OUT_DIR.mkdir(parents=True, exist_ok=True)

FILES = [
    ("工事", RAW_DIR / "chugoku_shikoku_kouji.html"),
    ("業務", RAW_DIR / "chugoku_shikoku_gyoumu.html"),
]
...
for kind, fp in FILES:
    ...
    for table in tables:
        ...
        for _, r in df.iterrows():
            ...
            company = r.iloc[1]
            corp = r.iloc[2]
            amount = clean_money(r.iloc[3])
            planned = clean_money(r.iloc[4])

            try:
                rate = float(str(r.iloc[5]).replace("%",""))
            except:
                rate = 0
            ...
            item = {
                ...
                "note": "",
                "bureau": "中国四国防衛局",
                "source_url": "https://www.mod.go.jp/rdb/chushi/contract/result/2024kekkak/",
                "start": start,
                "end": end
            }
...
(OUT_DIR / "chugoku_shikoku_all_cases.json").write_text(json.dumps(all_cases, ensure_ascii=False, indent=2))
(OUT_DIR / "chugoku_shikoku_top30.json").write_text(json.dumps(top30, ensure_ascii=False, indent=2))
(OUT_DIR / "chugoku_shikoku_company.json").write_text(json.dumps(company_list, ensure_ascii=False, indent=2))
(OUT_DIR / "chugoku_shikoku_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2))
