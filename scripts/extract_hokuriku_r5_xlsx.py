...
src = Path("data/mlit_r5_samples_unverified/04_hokuriku_kouji_2023.xlsx")
out = Path("data/mlit_r5_confirmed/hokuriku_r5_kouji_contracts.json")
...
for row in ws.iter_rows(min_row=5, values_only=True):
    ...
    records.append({
        ...
        "remarks": conv(row[18]),
        "is_awarded": conv(row[18]) in ["落札", "決定"],
    })

out.write_text(json.dumps(records, ensure_ascii=False, indent=2), encoding="utf-8")
print("saved:", out)
print("count:", len(records))
print("awarded_or_decided:", sum(1 for r in records if r["is_awarded"]))
