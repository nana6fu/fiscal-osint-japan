...
OUTPUT_JSON = "hokuriku_r7_all_full.json"
...
def main():
    print("=" * 70)
    print("北陸地方整備局 R7統合Excel → JSON抽出 v1")
    print("=" * 70)
    
    # 北陸R7: 月別Excel 24本を一括処理
    kouji_col_map = {
        'memo': 18,
        'contract_amount': 17,
    ...
    files = sorted([p for p in Path("r7_full").glob("*.xls") if not p.name.startswith("._")])
    kouji = []
    gyoumu = []

    for f in files:
        preview = pd.read_excel(f, header=None, nrows=8)
        text = " ".join(str(x) for x in preview.fillna("").values.flatten())

        if "gyoumu" in f.name.lower():
            gyoumu.extend(parse_excel(str(f), '業務', gyoumu_col_map))
        elif "kouji" in f.name.lower():
